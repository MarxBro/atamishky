...
<xsl:key name="rss_categories" match="entries/entry/bibliografia" use="." />
...
<xsl:sort />
	...
	<xsl:if test="$thisNode = $firstNodeAtSameLocation">
		...
		<li><a>
			...
			<xsl:attribute name="onclick">showCategory('bibliografia','<xsl:value-of select="."/>')</xsl:attribute>
			<xsl:value-of select="."/>
		</a></li>
	</xsl:if>
</xsl:for-each>
